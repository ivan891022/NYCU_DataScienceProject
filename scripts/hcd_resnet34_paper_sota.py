...
# -*- coding: utf-8 -*-
"""
HCD – ResNet34 (paper SOTA setting) on your 70/15/15 splits
...
Main setting we mimic:
    ...
    - Fine-tune ALL layers (ResNet34all)
    - Input: H&E patches (96x96), random crop to 90x90
    - Geometric aug: random H/V flip
    - No color augmentation, only per-channel normalize

Your changes:
    - Use your stratified 70/15/15 splits:
...
Folder layout (same as你現在的):
    hcd/
      train/                # Kaggle train images (220,025)
      test/                 # Kaggle test images (for submission)
      train_labels.csv      # Kaggle official labels
      sample_submission.csv
      splits/seed42_70_15_15/
          train.csv
          val.csv
          test.csv
"""
...
def build_train_transform(crop_size: int):
    ...
    Paper augment:
        ...
        - channel-wise normalize (我們用 ImageNet mean/std)
    """
    tf_list = [
...
def build_model(
    name: str = "resnet34",  # ResNet-34, ImageNet-1k
    drop_rate: float = 0.0,
    drop_path_rate: float = 0.0,
):
    """
    Paper: ResNet34 pre-trained on ImageNet, fine-tune all layers.
...
def main(args):
    ...
    # Model: ResNet34, fine-tune ALL layers
    model = build_model(
        name=args.model,
    ...
    if args.use_focal:
        ...
        if is_main():
            print("[INFO] Using FocalLoss (paper 沒特別寫，我們這裡提供選項)")
    else:
    ...
    # LR scaling (跟你EffNet script 一樣邏輯)
    ...
    if is_main():
        ...
        print("[INFO] backbone=ResNet34 (ResNet34all)")
        print(
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out_dir", default="outputs/resnet34_paper_sota")
    ...
    ap.add_argument(
        "--sub_name",
        type=str,
        default="submission_resnet34_paper_sota",
