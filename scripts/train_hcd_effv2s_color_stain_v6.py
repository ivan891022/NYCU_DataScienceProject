...
# -*- coding: utf-8 -*-
"""
HCD EfficientNetV2-S (1k) + 顏色增強 v6（更穩定）+ 溫和幾何增強 (DDP)

- data_dir:
    hcd                 -> 原始 HCD
    hcd_macenko_eda_v2  -> 你的 EDA-guided Macenko v2

- 實驗軸：
    1) 有 / 無 染色標準化（改 --data_dir）
    2) 有 / 無 顏色增強（加上 --no_color_aug 就是「無顏色增強」）

- 幾何增強 v6：只保留水平/垂直 flip + 小角度 Rotate
- 顏色增強 v6：更穩定、偏小幅度
    * OneOf 兩條分支（全部都以「接近 identity」為中心）：
        - bc_only       : BC ±6%
        - bc_gamma      : BC ±4% + Gamma(96~104)
  目的：讓 brightness / contrast 稍微變寬，但中位數保持接近；
       不再動 rb_ratio（沒有 RGBShift），降低破壞染色訊息的機率。

另外增加 --pos_weight_scale，讓你可以把 positive class 權重放大（例如 2.0）。
"""
...
class HCDDataset(Dataset):
    def __init__(
        ...
        if train:
            ...
            # 顏色增強 v6：保守版 BC/Gamma（不動 rb_ratio）
            color_tf = A.OneOf(
                [
                    # 1) bc_only：亮度/對比 ±6%
                    ...
                    # 2) bc_gamma：亮度/對比 ±4% + Gamma(96~104)
                    A.Compose(
                        [
                            A.RandomBrightnessContrast(
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out_dir", default="outputs/effv2s_color_stain_v6")
    ap.add_argument("--model", default="tf_efficientnetv2_s")

    # per-GPU batch 64，搭配 grad_accum_steps=4 (8GPU 時 global = 64*8*4 = 2048)
