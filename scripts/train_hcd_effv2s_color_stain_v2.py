...
# -*- coding: utf-8 -*-
"""
HCD EfficientNetV2-S + 顏色增強 + 幾何增強 (DDP, v2)

- 搭配「離線 Macenko 染色標準化」資料夾使用：
    --data_dir hcd                -> 原始 HCD
    --data_dir hcd_macenko_eda_v2 -> 你的 EDA-guided Macenko v2

- 實驗軸：
    1) 有 / 無 染色標準化（改 --data_dir）
    2) 有 / 無 顏色增強（加上 --no_color_aug 就是「無顏色增強」）
- 幾何增強：flip + rotate + 小幅 ShiftScaleRotate + 少量模糊/噪聲
"""
...
class HCDDataset(Dataset):
    def __init__(
        ...
        if train:
            # ---- 幾何增強：適中、不要太暴力 ----
            geo_transforms = [
                # flip + 旋轉（HCD patch 本身沒有方向性，這些都合理）
                A.HorizontalFlip(p=0.5),
                A.VerticalFlip(p=0.5),
                A.RandomRotate90(p=0.5),
                # 小幅 shift / scale / rotate
                A.ShiftScaleRotate(
                    shift_limit=0.03,
                    scale_limit=0.07,
                    rotate_limit=10,
                ...
                # 固定 resize 到輸入大小（不再做 RandomResizedCrop）
                A.Resize(self.img_size, self.img_size),
                # 少量模糊 + noise 模擬 scanner / focus 差異
                A.OneOf(
                    [
            ...
            color_block = [
                ...
                # 輕微 channel 抖動，避免學到固定 RGB shortcut
                A.RGBShift(
                    r_shift_limit=4,
                    g_shift_limit=4,
                    b_shift_limit=4,
            ...
            # val / test：只做 Resize + Normalize（保持顏色乾淨，好比較）
            self.tf = A.Compose(
                [
                    A.Resize(self.img_size, self.img_size),
                    A.Normalize(
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out_dir", default="outputs/effv2s_color_stain_v2")
    ap.add_argument("--model", default="tf_efficientnetv2_s_in21k")

    # per-GPU batch，建議用 64，搭配 grad_accum_steps=4 (global = 64*7*4=1792)
