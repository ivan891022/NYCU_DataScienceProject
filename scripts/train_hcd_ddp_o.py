...
def main(args):
    ...
    tr = pd.read_csv(split_dir/"train.csv"); val = pd.read_csv(split_dir/"val.csv")
    ...
    for ep in range(1, args.epochs+1):
        ...
        # 每個 epoch 在 val 上看 AUROC/F1（閾值自動找）
        au, f1, thr = evaluate(model, val_dl, device)
        if is_main():
            print(f"  -> val AUROC={au:.4f}  F1*={f1:.4f}  thr={thr:.3f}")
        if au > best["auroc"]:
    ...
    # 最後在 test 上報最終分數
    au, f1, thr = evaluate(model, test_dl, device)
    if is_main():
        print(f"[TEST] AUROC={au:.4f}  F1*={f1:.4f}  thr={thr:.3f}")
        print(f"[BEST(val)] epoch={best['epoch']} AUROC={best['auroc']:.4f} F1*={best['f1']:.4f} thr={best['thr']:.3f}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--lr_scale_ref", type=int, default=2048) # 例如 2048 → 全域 batch 2048 時用 base_lr
