...
# -*- coding: utf-8 -*-
"""
HCD EfficientNetV2-S (1k) + 顏色增強 v3 + 溫和幾何增強 (DDP)
...
- 幾何增強 v3：只保留水平/垂直 flip + 小角度 Rotate（不再做 ShiftScaleRotate / RandomResizedCrop）
- 顏色增強 v3：
    * RandomBrightnessContrast ±10%
    * RandomGamma(90 ~ 110)
    * 很小的 RGBShift (±3)
  => 讓 brightness / contrast / rb_ratio 分布略微拉寬，但中位數幾乎不變
"""
...
class HCDDataset(Dataset):
    def __init__(
        ...
        if train:
            ...
            # ---- 顏色增強 v3：亮度 / 對比 / gamma + 很小的 RGB shift ----
            color_block = [
...
# ----------------- model / metric -----------------
def build_model(
    name: str = "tf_efficientnetv2_s",  # 1k 版本
    drop_rate: float = 0.3,
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out_dir", default="outputs/effv2s_color_stain_v3")
    ...
    ap.add_argument("--lr_scale_ref", type=int, default=2048)  # 64*8*4
